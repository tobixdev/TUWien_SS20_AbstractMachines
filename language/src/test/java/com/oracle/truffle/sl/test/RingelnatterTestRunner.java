...
import com.oracle.truffle.sl.test.RingelnatterTestRunner.TestCase;

public class RingelnatterTestRunner extends ParentRunner<TestCase> {
    ...
    public RingelnatterTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        RingelnatterTestSuite suite = c.getAnnotation(RingelnatterTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", RingelnatterTestSuite.class.getSimpleName(), c.getName(), RingelnatterTestRunner.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != RingelnatterTestSuite.class) {
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            ...
            Source source = Source.newBuilder(RingelnatterLanguage.ID, path.toFile()).interactive(true).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        RingelnatterTestRunner suite = new RingelnatterTestRunner(testClass);
        if (args.length > 0) {
